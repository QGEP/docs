...
.. note:: If a cover level changes, the Z value of the cover's geometry will be ajusted. When the geometry changes, the level is ajusted as well.  If both values change, the level takes precendence. The same situation is on editing the wastewater node directly. On this main layer wastewater structure, the only way to set the Z value of the geometry is to set the levels of cover and wastewater node.